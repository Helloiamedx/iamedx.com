...
export type DriftWallProps = {
  ...
  perspective?: number;
  depth?: number;
  /** Uniform plane scale — bump to fill wide viewports. */
  scale?: number;
  /** Grow column count so the plane spans the container width. */
  fillWidth?: boolean;
  speed?: number;
  direction?: "up" | "down";
  variance?: number;
...
export function DriftWall({
  ...
  const columnItems = useMemo(() => {
    ...
    items.forEach((item, i) => {
      cols[i % resolvedColumns]!.push(item);
    });
  ...
  const baseVelocities = useMemo(() => {
    ...
    // Same speed; alternate direction so adjacent columns are staggered.
    return columnItems.map((_, c) => {
      const altSign = c % 2 === 0 ? 1 : -1;
      return speed * dirSign * altSign;
    });
  }, [columnItems, speed, direction]);

  useEffect(() => {
    offsetsRef.current = columnMeta.map((meta, c) => {
      // Vertical phase offset between columns (错开)
      const phase = (c % 2 === 0 ? 0 : 0.5) + (c * 0.12) % 0.35;
      return meta.copyHeight * phase;
    });
