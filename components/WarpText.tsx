...
function getFontValue(value: number | string) {
...
function wrapLines(
  ...
  paragraphs.forEach((paragraph) => {
    ...
    for (let i = 1; i < words.length; i++) {
      ...
      if (measureLine(ctx, next, letterSpacing) <= maxWidth) {
        current = next;
      } else {
        lines.push(current);
        current = words[i];
...
function buildTextCanvas({
  ...
  let lines = wrapLines(ctx, props.text, maxWidth, letterSpacing);
  let blockHeight = Math.max(lineHeight * Math.max(lines.length, 1), 1);

  // Shrink only if wrapped block still overflows vertically.
  if (blockHeight > maxHeight) {
    const fit = maxHeight / blockHeight;
    fontSizePx *= fit;
    letterSpacing *= fit;
    lineHeight *= fit;
    applyFont();
    lines = wrapLines(ctx, props.text, maxWidth, letterSpacing);
    blockHeight = Math.max(lineHeight * Math.max(lines.length, 1), 1);
  }
...
export function WarpText({
  ...
  const propsRef = useRef<WarpProps>({
    text,
    color,
    fontSize,
    fontWeight,
