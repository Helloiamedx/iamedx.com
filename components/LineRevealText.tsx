 ...
 *
 * Uses useLayoutEffect (not useGSAP context) so cleanup cannot hit the
 * nested Context.getTweens infinite recursion that crashed the home page.
 */
export function LineRevealText({ text, className }: LineRevealTextProps) {
  ...
  useLayoutEffect(() => {
    ...
    /*
     * Services phase labels: observe the title itself so each one plays when
     * it enters view (and sticky phase labels are not wiped mid-stage).
     */
    const selfTrigger = el.closest(".svc-phase__label");
